...
% Beth modified for her IMEC data 12.3.17

% Must run this file on Beefcake (nidq.bin files are > 50 GB) 
% STOP AT LINE 114!
...
%saveDirectory = 'Z:\Shared\Beth';
saveDirectory = 'C:\Users\labadmin\Desktop\jrclust';

%binName =  'Vgateight_17_04_27_g0_t0.nidq.bin';                          % .bin filename (e.g. 'JP_WR16_2130um_dura_062117_g0_t0.nidq.bin')
binName =  'Vgateight_170428_g0_t0.nidq.bin';
cd(fileDirectory)
...
trialStartCh = 193; XposCh = trialStartCh;
...
for i = 0:totalTimeSecs-1 % read second-by-second incrementally to avoid a memory issue
    ...
    Xpos(1,i*1000+1:(i+1)*1000) = tempXpos; % accumulated the decimated data(trialStarts)
    Ypos(1,i*1000+1:(i+1)*1000) = tempYpos; % accumulated the decimated data(wheel)
...
save('BehVariables2', 'Xpos', 'Ypos', 'lick', 'sole');   % save relevant behavioral variables


% load('BehVariables', 'Xpos', 'Ypos', 'lick', 'sole'); % load relevant behavioral variables
figure; hold on;
%plot(Ypos(1:1000000), 'r');plot(Xpos(1:1000000));
plot(Ypos, 'r');
plot(Xpos);
...
%% JUNCHOL'S CODE FOR CALLING REACH PARAMS:
[ reachStart, reachStop, reach0, pos1, pos2 ] = getReachTimesJPBeth1st( positionData );     % all reach traces, aligned to start (pos1), to stop (pos2)
...
save('BehVariables2', 'Xpos', 'Ypos', 'lickTraces', 'sole', 'reach0', 'pos1', 'pos2', 'vel1', 'vel2', 'ts') % append the position/velocity data variables
