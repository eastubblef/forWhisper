...
for j = 1:length(units) % # of units
    ...
    delta = zeros(1,ceil(popData{1,j}(numStamps))); % delta function of the spike train
...
%     [respCSS, psthZscore(j,:)] = TNC_AlignRasters(tmpSmooth, popData{1,j}, -1, alignVar1, psthWin, 1, 1); % actual zscore normalized psth % alignVar1(:,i) needs to be all trials of an event of interest
    [respCSS] = TNC_AlignRasters(tmpSmooth, popData{1,j}, -1, alignVar1, psthWin, 1, 1); % actual zscore normalized psth % alignVar1(:,i) needs to be all trials of an event of interest
