function [ binSpkCount ] = psthBIN( neural, behav, binsize, psthWin, stableTrials )
...
%     % overall average, std FR
%     % overall mean and std rate
%     if alignBehVar(length(alignBehVar)) > numel(delta)
%         stdRate     = std(delta(alignBehVar(1):numel(delta)));
%         meanRate     = mean(delta(alignBehVar(1):numel(delta)));
%     else
%         stdRate     = std(delta(alignBehVar(1):alignBehVar(length(alignBehVar))));
%         meanRate     = mean(delta(alignBehVar(1):alignBehVar(length(alignBehVar))));
%     end
